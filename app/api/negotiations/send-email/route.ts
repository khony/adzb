...
export async function POST(request: NextRequest) {
  try {
    ...
    const { negotiationId, organizationId } = body

    if (!negotiationId || !organizationId) {
      return NextResponse.json(
        { error: 'negotiationId e organizationId são obrigatórios' },
        { status: 400 }
      )
    }
...
<body>
  <div class="container">
    ...
    if (emailError) {
      ...
      return NextResponse.json({ error: 'Erro ao enviar email', details: emailError }, { status: 500 })
    }
